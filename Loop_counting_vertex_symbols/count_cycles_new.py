...
##    all_node_list=list(G.nodes) # equivalent to list(G)
    ...
    cpu = 48#os.cpu_count()-1  # No. of parallel processors
...
##    plt.plot(np.arange(1,max_loop_order+1), (loop_order_count_mean_field[1:]),'bo-') # check this ceil() function!!- i am assuming this is the correct implementation, but not sure
    ...
    plt.savefig('mean_field_loop_order_dist_new',bbox_inches='tight')
    np.savetxt('loop_order_mean_field_new.txt',np.array(np.transpose([np.arange(1,max_loop_order+1), (loop_order_count_mean_field[1:]),(loop_fraction_mean_field[1:])])),header='loop_order,count,fraction')
    ...
    G_conn = [G.subgraph(c).copy() for c in largest_cc]# nx.connected_components(G)]

    e = G_conn.number_of_edges()
    v = G_conn.number_of_nodes()
    c = nx.number_connected_components(G_conn)
    print('e_conn', e)
    print('v_conn', v)
    print('c_conn', c)
    cycle_rank = e - v + c
    print('cycle rank_conn=', cycle_rank)
    '''
    ##plt.show()
    ...
    ##    P_vertex[symbol_str(all_vertex_symbols[0])]=1
    ##    P_vertex_trunc[symbol_str(all_vertex_symbols[0])]=1
    ...
    if(len(all_vertex_symbols)>0):
        ...
        for i in P_vertex_sorted.keys():
            if(i!=''):
                _=plt.plot(i,P_vertex_sorted[i],'o',color=c)#,width=1.0)
    ##            plt.bar(i,P_vertex_sorted[i],color=c,width=1.0)
                c = next(color)
...
##                            stop
    ...
    ##for i in range(0,len(all_vertex_symbols)):
    ##    print(str(main_node_list[i]),':',symbol_str(all_vertex_symbols[i],main_node_list[i]))
    end=timer()
